...
test.describe('Multiplayer P2P Flow', () => {
  ...
  test('Host creates room and Player joins', async () => {
    ...
    // Wait for Room Code
    await expect(hostPage.getByText('Room:', { exact: false })).toBeVisible({
      timeout: 10000,
    });
    const roomCodeText = await hostPage
      .getByText('Room:', { exact: false })
      .textContent();
    ...
    // Host sees player
    await expect(hostPage.getByText('JoinerUser')).toBeVisible({
      timeout: 10000,
    });
    // Player sees lobby
    ...
    // Select Quiz
    await hostPage
      .getByText('Advanced Finnish Language & Complex Grammar')
      .first()
      .click();
