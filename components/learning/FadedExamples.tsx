...
export const FadedExamples: React.FC<FadedExamplesProps> = ({
      ...
      if (quiz.questions && quiz.questions.length >= 2) {
        ...
        for (let i = 0; i < Math.min(quiz.questions.length, 6); i += 2) {
          ...
          if (q1 && q1.explanation && q1.answer) {
            ...
            const codeWithBlank = q1.explanation.replace(
              keyTerm,
              '___BLANK___',
            );
  ...
  const renderCodeWithBlanks = () => {
    ...
    return lines.map((line, lineIdx) => {
      if (line.includes('___BLANK___')) {
        ...
        const isCorrect =
          showResult &&
          userValue.trim().toLowerCase() === blank.answer.toLowerCase();
        const isWrong =
          showResult &&
          userValue.trim().toLowerCase() !== blank.answer.toLowerCase();
          ...
          <View key={lineIdx} style={styles.codeLine}>
            ...
            <View style={styles.lineContent}>
              ...
              <TextInput
                ...
                value={userValue}
                onChangeText={(text) =>
                  handleInputChange(currentBlankIndex, text)
                }
                placeholder="fill in..."
  ...
  if (completed) {
    const totalStagesWithBlanks = examples.reduce(
      (acc, ex) => acc + ex.stages.filter((s) => s.blanks.length > 0).length,
      0,
    );
      ...
      <View style={styles.container}>
        <View style={styles.completedState}>
          ...
          <Text style={styles.scoreText}>
            {correctCount} / {totalStagesWithBlanks} stages correct (
            {percentage}%)
          </Text>
          <Text style={styles.encouragementText}>
            {percentage >= 90
              ? "You've mastered these patterns! 🏆"
              : percentage >= 70
              ? 'Great understanding! 🎯'
      ...
      >
        ...
        <View style={styles.codeCard}>
          <View style={styles.codeHeader}>
            ...
            <View
              style={[styles.codeHeaderDot, { backgroundColor: '#FFBD2E' }]}
            />
            <View
              style={[styles.codeHeaderDot, { backgroundColor: '#27C93F' }]}
            />
          ...
          >
            <View style={styles.codeInner}>{renderCodeWithBlanks()}</View>
          </ScrollView>
        </View>
        ...
        {showResult && (
          <View
            style={[
              styles.resultCard,
              allCorrect ? styles.correctResult : styles.wrongResult,
            ]}
          >
              ...
              <View style={styles.correctAnswers}>
                ...
                {currentStage.blanks.map((blank, idx) => (
                  <Text key={idx} style={styles.correctAnswerText}>
                    {idx + 1}.{' '}
                    <Text style={styles.codeInline}>{blank.answer}</Text>
                  </Text>
                ))}
              </View>
            )}
