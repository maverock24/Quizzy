...
export const ParsonsProblems: React.FC<ParsonsProblemsProps> = ({
        ...
        quiz.questions.forEach((q: QuizQuestion, idx: number) => {
          if (q.explanation && q.explanation.length > 50) {
            ...
            if (sentences.length >= 3) {
              const lines: ParsonsLine[] = sentences.map(
                (s: string, i: number) => ({
                  id: `line-${i}`,
                  code: s.trim() + '.',
                  indent: 0,
  ...
  const handleLineTap = (line: DraggableLine, isFromPlaced: boolean) => {
    ...
    if (isFromPlaced) {
      ...
      setPlacedLines((prev) => [
        ...prev,
        { ...line, isPlaced: true, position: prev.length },
      ]);
    }
  };
  ...
  const checkAnswer = () => {
    ...
    const correct =
      JSON.stringify(placedIds) === JSON.stringify(problem.correctOrder);
        ...
        <View style={styles.availableContainer}>
          ...
          {availableLines.length === 0 && !showResult && (
            <Text style={styles.allPlacedText}>
              All lines placed! Check your answer.
            </Text>
          )}
        </View>
