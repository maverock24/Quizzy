 ...
 */
export const loadSRSData = async (): Promise<
  Record<string, SRSQuestionData>
> => {
  try {
...
export const saveSRSData = async (
  data: Record<string, SRSQuestionData>,
): Promise<void> => {
  try {
...
export const getDueQuestions = async (
    ...
    for (const quiz of quizzes) {
      ...
      for (const question of quiz.questions) {
        ...
        if (!allSRSData[questionId] && !existingIds.has(questionId)) {
          const newSRSData = getQuestionSRSData(
            {},
            quiz.name,
            question.question,
          );
  ...
  dueQuestions.sort(
    (a, b) => a.srsData.nextReviewDate - b.srsData.nextReviewDate,
  );
...
export const getSRSStats = async (): Promise<{
  ...
  return {
    ...
    dueToday: values.filter((q) => q.nextReviewDate <= endOfDay.getTime())
      .length,
