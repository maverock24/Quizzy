...
export function useReadAloud() {
  ...
  useEffect(() => {
    if (Platform.OS === 'web' && 'speechSynthesis' in window) {
      ...
      const loadVoices = () => {
        ...
        console.log(
          `Voice loading attempt ${voicesLoadAttempts + 1}: Found ${
            voices.length
          } voices`,
        );
  ...
  const readAloud = useCallback(
    (
      ...
      console.log(
        'ReadAloud called with text length:',
        text.length,
        'Mobile:',
        isMobile.current,
      );
      console.log(
        'Current selectedVoice from context:',
        selectedVoice
      ...
      if (Platform.OS === 'web' && 'speechSynthesis' in window) {
          ...
          if (voices.length === 0 && !voicesLoaded.current) {
            ...
            const retryVoiceLoad = () => {
              ...
              if (currentVoices.length > 0 || retryCount >= maxRetries) {
                console.log(
                  `Voice loading ${
                    currentVoices.length > 0 ? 'successful' : 'timed out'
                  }`,
                );
        ...
        const proceedWithSpeech = (voices: SpeechSynthesisVoice[]) => {
          ...
          ): { type: 'text' | 'pause'; content: string | number }[] => {
            const segments = textWithBreaks.split(
              /(<break time="\d+s"\s*\/?>)/,
            );
            const result: {
              type: 'text' | 'pause';
          ...
          const splitIntoChunks = (
            textToSplit: string,
            maxLen = isMobile.current ? 100 : 200,
          ): string[] => {
          ...
          voices.forEach((v, i) =>
            console.log(
              `${i}: ${v.name} (${v.lang}) ${v.default ? '[DEFAULT]' : ''}`,
            ),
          );
          ...
          if (!chosenVoice) {
            ...
            if (isMobile.current) {
              ...
              if (languageVoices.length > 0) {
                ...
                console.log(
                  'Mobile: Using first language voice:',
                  chosenVoice.name,
                );
              } else {
                ...
                console.log(
                  'Mobile: Using fallback voice:',
                  chosenVoice?.name || 'None',
                );
              }
            } else {
              // Desktop logic
              chosenVoice =
                languageVoices[voiceIndex] ||
                voices[voiceIndex] ||
                voices[0] ||
                null;
            }
          ...
          segments.forEach((segment) => {
            if (segment.type === 'text') {
              ...
              chunks.forEach((chunk) => {
                ...
                utterance.rate = isMobile.current
                  ? Math.max(0.5, rate - 0.1)
                  : rate; // Slightly slower on mobile
                utterance.pitch = 1;
                utterance.volume = 1;
          ...
          console.log(
            `Created ${utteranceCount} utterances for speech synthesis`,
          );
