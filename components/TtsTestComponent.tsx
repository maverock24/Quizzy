...
export const TtsTestComponent: React.FC = () => {
  ...
  const testBasicTts = () => {
    const testText =
      'Hello, this is a test of text to speech functionality on Chrome mobile.';
    readAloud(testText, undefined, 1.0, 0);
  };

  const testWithBreaks = () => {
    const testText =
      'This is a test. <break time="2s"/> After a two second pause. <break time="3s"/> After a three second pause.';
    readAloud(testText, undefined, 0.9, 0);
  };

  const testLongText = () => {
    const longText =
      'This is a longer text to test chunking functionality. It contains multiple sentences to ensure that the text-to-speech system properly handles longer content. The system should break this into appropriate chunks and speak them sequentially without interruption.';
  ...
  const checkSpeechSynthesis = () => {
    if ('speechSynthesis' in window) {
      ...
      debugInfo += `Default voice: ${
        voices.find((v) => v.default)?.name || 'None'
      }\n`;
      debugInfo += `Speaking: ${
        window.speechSynthesis.speaking ? 'Yes' : 'No'
      }\n`;
      debugInfo += `Pending: ${
        window.speechSynthesis.pending ? 'Yes' : 'No'
      }\n`;
