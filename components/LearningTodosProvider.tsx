...
const LearningTodosContext = createContext<
  LearningTodosContextType | undefined
>(undefined);
...
export const LearningTodosProvider: React.FC<{ children: ReactNode }> = ({
  children,
}) => {
  ...
  const getTodoByQuestion = useCallback(
    (question: string): LearningTodoItem | undefined => {
...
export const useLearningTodos = (): LearningTodosContextType => {
  ...
  if (!context) {
    throw new Error(
      'useLearningTodos must be used within a LearningTodosProvider',
    );
  }
  return context;
};
